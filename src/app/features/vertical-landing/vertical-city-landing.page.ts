...
import { CITY_CONFIGS, CityConfig } from './city-landing.data';
...
/**
 * Combina la configuración de un vertical con una ciudad para generar una
 * landing de cola larga ("agenda para barberías en Madrid") sin duplicar
 * ninguna de las secciones compartidas — mismo principio que las landings
 * por vertical: un único motor, solo cambia el texto de marketing.
 */
function buildCityVerticalConfig(vertical: VerticalConfig, city: CityConfig): VerticalConfig {
  return {
    ...vertical,
    metaTitle: `Agenda y WhatsApp para tu ${vertical.businessNameSingular} en ${city.displayName} — Agendia`,
    metaDescription: `${vertical.metaDescription} Ideal para ${vertical.businessNameSingular}s en ${city.displayName}.`,
    headline: `Gestiona las citas de tu ${vertical.businessNameSingular} en ${city.displayName}`,
  };
}

@Component({
  selector: 'app-vertical-city-landing-page',
  standalone: true,
  imports: [
    CommonModule,
    RouterLink,
  ...
  template: `
    @if (config) {
      ...
      <div class="vertical-landing__not-found">
        <app-empty-state icon="🔍" title="Página no encontrada" description="Todavía no existe una página para esta combinación.">
...
export class VerticalCityLandingPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    const verticalSlug = this.route.snapshot.data['vertical'] as string;
    const citySlug = this.route.snapshot.paramMap.get('city') ?? '';

    const vertical = VERTICAL_CONFIGS[verticalSlug];
    const city = CITY_CONFIGS[citySlug];

    this.config = vertical && city ? buildCityVerticalConfig(vertical, city) : null;
